 ...
 * @LastEditTime: 2022-05-13 16:17:40
...
cv::Mat matchPointCloudsID(std::map<int, Eigen::Vector3d>& ids_points_img0, std::map<int, Eigen::Vector3d>& ids_points_img1, PointCloudT::Ptr cloud_match)
{
...
void getPointCloudInFOV(PointCloudT::Ptr cloud_in, const Eigen::Matrix4d& img_T_w, std::map<int, Eigen::Vector3d>& ids_points_FOV, PointCloudT::Ptr cloud_FOV)
{
    ...
    for (int i = 0; i < transformed_cloud->points.size(); i++) {
        if (transformed_cloud->points[i].z > 0) {
            Eigen::Vector3d transformed_point(transformed_cloud->points[i].x, transformed_cloud->points[i].y, transformed_cloud->points[i].z);
            Eigen::Vector2d pixel;
            ...
            if (pixel.x() >= 0 && pixel.x() < m_camera->imageWidth() && pixel.y() >= 0 && pixel.y() < m_camera->imageHeight()) {
                if (depth_img.at<ushort>((int)pixel(1), (int)pixel(0)) > transformed_point.norm() * 1000) {
                    ...
                    ids_points_FOV[i] = Eigen::Vector3d(transformed_cloud->points[i].x, transformed_cloud->points[i].y, transformed_cloud->points[i].z);
    ...
    while (!viewer.wasStopped()) { // Display the visualiser until 'q' key is pressed
        if (cv::waitKey(100) != 255 || is_first) {
            ...
            if (gt_data.find(image_ids[img0_id]) != gt_data.end() && gt_data.find(image_ids[img1_id]) != gt_data.end()) {
                ...
                cv::Mat image_pair;
                cv::hconcat(cv::imread(image_path + "/" + std::to_string(image_ids[img0_id]) + ".png", cv::IMREAD_GRAYSCALE), cv::imread(image_path + "/" + std::to_string(image_ids[img1_id]) + ".png", cv::IMREAD_GRAYSCALE), image_pair);
