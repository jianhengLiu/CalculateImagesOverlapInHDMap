...
void extractPointCloudAtGivenHeight(PointCloudT::Ptr cloud, PointCloudT::Ptr filtered_cloud,
                                    float height)
{
...
Eigen::Matrix4f iterativeICP(PointCloudT::Ptr cloud_target, PointCloudT::Ptr cloud_input,
                             PointCloudT::Ptr cloud_output)
{
    pcl::IterativeClosestPoint<PointT, PointT> icp;
    icp.setInputTarget(cloud_target);
    icp.setTransformationEpsilon(
        1e-8);  //设置前后两次迭代的转换矩阵的最大容差（epsilion），一旦两次迭代小于这个最大容差，则认为已经收敛到最优解，迭代停止。迭代停止条件之二，默认值为：0
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Loaded file " << ply_path << " (" << cloud_in->size() << " points) in "
              << time.toc() << " ms\n"
              << std::endl;
    ...
    std::cout << "w_T_c1:" << std::endl << w_T_c1.matrix() << std::endl;
    ...
    time.tic();
    Eigen::Matrix4f w_T_i =
        iterativeICP(target_grid_cloud, transformed_input_grid_cloud, match_cloud);
    std::cout << "iterativeICP cost: " << time.toc() << " ms\n" << std::endl;
    std::cout << "w_T_i:" << std::endl << w_T_i << std::endl;
    ...
    pcl::visualization::PointCloudColorHandlerCustom<PointT> transformed1_cloud_color_handler(
        target_grid_cloud, 230, 20, 20);  // Red
    ...
    pcl::visualization::PointCloudColorHandlerCustom<PointT> transformed2_cloud_color_handler(
        transformed_input_grid_cloud, 20, 230, 20);  // Red
    viewer.addPointCloud(transformed_input_grid_cloud, transformed2_cloud_color_handler,
                         "transformed_grid_cloud");
    viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2,
                                            "transformed_grid_cloud");

    pcl::visualization::PointCloudColorHandlerCustom<PointT> input_grid_cloud_color_handler(
        input_grid_cloud, 255, 0, 255);  // Red
    viewer.addPointCloud(input_grid_cloud, input_grid_cloud_color_handler, "input_grid_cloud");
    viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2,
                                            "input_grid_cloud");

    pcl::visualization::PointCloudColorHandlerCustom<PointT> match_cloud_color_handler(match_cloud,
                                                                                       20, 20, 230);
